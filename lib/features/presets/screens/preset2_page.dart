...
class Preset2Page extends StatefulWidget {
  ...
  const Preset2Page({super.key, required this.presetData});

  @override
  _Preset2PageState createState() => _Preset2PageState();
}

class _Preset2PageState extends State<Preset2Page> {
  ...
  Future<void> _loadPresetData() async {
    final presetData = widget.presetData['preset2']?['presetData']?[0];
    if (presetData != null) {
      setState(() {
  ...
  Future<void> _savePresetData() async {
    ...
    final file = File('${directory.path}/preset_2_prototype.json');
    await file.writeAsString(jsonEncode(widget.presetData));

    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text("'Preset 2' Successfully Saved!")),
    );
  }
